...
def run_demo_with_params(strategy="simple", **params):
    ...
    if strategy == "avellaneda":
        mm = AvellanedaMarketMaker(symbol='X', size=1.0, base_spread=params.get('base_spread', 0.01), risk_aversion=params.get('risk_aversion', 0.1))
        eng.register_strategy(mm)
    elif strategy == "pairs":
        pairs = PairsStrategy(symbol_x='X', symbol_y='Y', window=params.get('window', 100), entry_z=params.get('entry_z', 2.0), exit_z=params.get('exit_z', 0.5), quantity=params.get('quantity', 100.0))
        eng.register_strategy(pairs)
    else:
        mm = SimpleMarketMaker(symbol='X', base_spread=params.get('base_spread', 1.0), inventory_coeff=params.get('inventory_coeff', 0.1))
        eng.register_strategy(mm)
    eng.run_demo()
...
def sweep_and_save(strategy="simple", param_combos=None, out_csv="sweep_results.csv"):
    if param_combos is None:
        if strategy == "pairs":
            param_combos = itertools.product([50, 100, 150], [1.0, 1.5, 2.0], [0.3, 0.5, 0.8], [50, 100, 200], [0.0000, 0.0005], [0.0, 0.0001])
